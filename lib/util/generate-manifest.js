...
const parse_defapi_base_dir_1 = __importDefault(require("./parse-defapi-base-dir"));
...
function generateManifest(input, app) {
    ...
    if (!fs_extra_1.default.existsSync(srcDir)) {
        throw new errors_1.DefapiError('Unable to resolve src_path');
    }
    ...
    let defaultDict = {};
    let manifestDict = {};
    let defaultDefs = api_1.getEndpoints(app);
    let mergedDefs = [];
    let manifestDefs = [];
    ...
    mergedDefs = parse_defapi_base_dir_1.default(defapiBaseDir, mergedDefs);
    let contents = `/**
